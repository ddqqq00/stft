...
from test_model import (
    ...
    GATFeatureExtractor, 
    MLPFeatureExtractor, 
    DualPathFusionModel_watkins
)

def parse_args():
    parser = argparse.ArgumentParser(description="Ablation Study: GAT vs MLP on Watkins Dataset")
    ...
    parser.add_argument('--weight_decay', type=float, default=1e-2, help='Weight decay (Strong regularization)')
    parser.add_argument('--num_epochs', type=int, default=50, help='Number of training epochs')
    parser.add_argument('--top_k_classes', type=int, default=6, help='Select top k classes')
    parser.add_argument('--sr', type=int, default=40000, help='Audio sampling rate')
    
    # --- 核心消融参数 ---
    parser.add_argument('--gnn_type', type=str, default='gat', 
                        choices=['gat', 'mlp'], 
                        help="Ablation Mode: 'gat' (Graph Attention) or 'mlp' (Simple Concat)")
    
    # --- MLP 特定参数 (必须根据您的图数据设置!) ---
    parser.add_argument('--num_nodes', type=int, default=8, 
                        help="Fixed number of nodes per graph (Required for MLP)")
    parser.add_argument('--node_in_channels', type=int, default=79, 
                        help="Input dimension of each node (e.g., 79 for Watkins)")
...
def features_extractor(filename, sr):
    ...
    #     noise_amp = 0.005 * np.random.uniform(0.0, 1.0) * np.max(audio)
...
def calculate_precision(y_true, y_pred, num_classes):
    ...
    for i in range(num_classes):
        ...
        fp = cm[:, i].sum() - tp
        denominator = tp + fp
        precision_scores.append(tp / denominator) if denominator > 0 else precision_scores.append(0)
...
if __name__ == '__main__':
    ...
    # --- 打印消融实验模式 ---
    print(f"Running Ablation Study: GNN Type = {args.gnn_type}") 
    ...
    # 4. Initialize model (动态选择组件)
    
    # A. 初始化 CVCNN (始终相同)
    # 使用较强的 dropout (0.5) 防止过拟合
    cnn_extractor = ComplexCNNFeatureExtractor(dropout_rate=0.5).to(device)
    
    # B. 初始化 第二路 (根据参数选择 GAT 或 MLP)
    # 假设 GAT 隐藏层维度为 512，输出维度为 4096 (512 * 8 heads)
    GAT_HIDDEN = 512
    GAT_HEADS = 8
    LATE_DIM = GAT_HIDDEN * GAT_HEADS # 4096
    
    if args.gnn_type == 'gat':
        print(f"Initializing GAT Branch (In: {args.node_in_channels}, Hidden: {GAT_HIDDEN}, Out: {LATE_DIM})")
        ...
        print(f"Initializing MLP Branch (In: {args.node_in_channels}x{args.num_nodes}, Out: {LATE_DIM})")
        gat_or_mlp_extractor = MLPFeatureExtractor(
            in_channels=args.node_in_channels,
            num_nodes=args.num_nodes, 
            hidden_dim=1024, # MLP 中间层
            out_dim=LATE_DIM, # 确保输出维度与 GAT 一致 (4096)
            dropout_rate=0.5
        ).to(device)
    
    # C. 初始化 融合模型
    # 注意: cnn_late_dim 这里是 256 (因为我们使用了降维后的 ComplexCNNFeatureExtractor)
    cnn_late_dim = 256 
    
    model = DualPathFusionModel_watkins(
        cnn_extractor=cnn_extractor,
        gat_or_mlp_extractor=gat_or_mlp_extractor,
        cnn_late_dim=cnn_late_dim,
        gat_or_mlp_late_dim=LATE_DIM,
    ...
    for epoch in range(args.num_epochs):
        ...
        for stft_batch, graph_batch in pbar_train:
            ...
            pbar_train.set_postfix(loss=running_loss_cls / (pbar_train.n + 1), acc=100 * correct / total)
